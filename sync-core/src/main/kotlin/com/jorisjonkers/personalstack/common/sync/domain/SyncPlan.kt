 ...
 */
data class SyncPlan<A : Any, R : Any, RID : Any, KEY : Any>(
    val context: SyncContext<*>,
    val decisions: List<SyncDecision<A, R, RID, KEY>>,
) {
    /** All conflict decisions in the plan; these are never executed. */
    val conflicts: List<SyncDecision.Conflict<A, R, RID, KEY>>
        get() = decisions.filterIsInstance<SyncDecision.Conflict<A, R, RID, KEY>>()
